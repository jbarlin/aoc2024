...
namespace point
{
    ...
    const bool Point::can_move_sq(const Direction &dir, unsigned long size) const
    {
        return can_move(dir, size, size);
    }

    const bool Point::can_move(const Direction &dir, unsigned long x_limit, unsigned long y_limit) const
    {
        if (x != 0 || dir.x > -1)
        {
            if (y != 0 || dir.y > -1)
    ...
    const bool Point::inside(const unsigned long square_size) const
    {
    ...
    const std::vector<Point> Point::get_plus_in_bounds(const unsigned long square_size) const
    {
    ...
    const std::optional<Point> Point::continue_to_next_point(const Point &B, const unsigned long sq_size)
    {
        ...
        if (x >= 0 && ((unsigned long) x) < sq_size && y >= 0 && ((unsigned long) y) < sq_size)
        {
            const Point c(x, y);
    ...
    const std::set<Point> Point::create_line(const Point &B, const unsigned long sq_size)
    {
